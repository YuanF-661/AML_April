...
class DrumDataset(Dataset):
    def __init__(self, mfccs, waveforms, drum_types, drum_machines, num_drum_types=None, use_raw_audio=False):
        self.mfccs = mfccs
        self.waveforms = waveforms  # 添加原始波形存储
        self.drum_types = drum_types
        self.drum_machines = drum_machines
        self.num_drum_types = num_drum_types
        self.use_raw_audio = use_raw_audio  # 控制是否返回原始波形
    ...
    def __getitem__(self, idx):
        ...
        if self.use_raw_audio:
            ...
            if self.num_drum_types is not None:
                # 创建one-hot编码
        ...
        # 如果不需要原始波形
            ...
            if self.num_drum_types is not None:
                ...
                drum_type_onehot = torch.zeros(self.num_drum_types)
                drum_type_onehot[drum_type] = 1
                return mfcc, (drum_type, drum_machine, drum_type_onehot)

            # 否则，按原样返回
            return mfcc, (drum_type, drum_machine)
...
def create_dataset(data_path, sample_rate=22050, target_length=22050, train_ratio=0.7, val_ratio=0.15, test_ratio=0.15,
    ...
    # 创建数据集
    dataset = DrumDataset(mfcc_tensor, waveform_tensor, drum_types_tensor, drum_machines_tensor,
